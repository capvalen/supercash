...
if($totalRow==0){
	...
	while($row = mysqli_fetch_array($sql, MYSQLI_ASSOC))
	{
		...
		if($_COOKIE['ckPower']==6 ): $boton = "<button class='btn btn-sm btn-negro btn-outline btnEditarCajaMaestra'><i class='icofont icofont-edit'></i></button> <button class='btn btn-sm btn-azul btn-outline btnPrintCajaEsp' data-caja='{$row["idCaja"]}' ><i class='icofont icofont-print'></i></button> ";
		elseif( $_COOKIE['ckPower']==1 ):  $boton = "<button class='btn btn-sm btn-success btn-outline btnChangeMonedaEsp' data-caja='{$row["idCaja"]}' ><i class='icofont icofont-pen-alt-3'></i></button> ";
		else: $boton=''; endif; ?>
